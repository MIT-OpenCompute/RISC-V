...
class Main() extends Module {
    ...
    RAM.io.enable := false.B
    RAM.io.write := false.B
    RAM.io.addr := 0.U(10.W)
    RAM.io.data_in := 0.U(32.W)
    ...
    when(io.execute) {
        ...
        is ("b0000000_100_01100_11".U) {
            // XOR instruction https://msyksphinz-self.github.io/riscv-isadoc/html/rvi.html#xor
        ...
        is ("b00000_00_101_01100_11".U) {
            // SRL instruction https://msyksphinz-self.github.io/riscv-isadoc/html/rvi.html#srl
            ...
            // Execute SRL: x[rd] = x[rs1] >>u x[rs2]

            // Read rs1 and rs2 values
            regFile.io.read_addr_A := rs1
            val rs1_value = regFile.io.out_A
            regFile.io.read_addr_B := rs2
            val rs2_value = Cat(Fill(27,0.U), regFile.io.out_B(5,0))

            // Set up ALU
            alu.io.operation := "b1000".U // Logical right shift operation
            alu.io.signed := false.B
            alu.io.a := rs1_value
            alu.io.b := rs2_value
            val alu_result = alu.io.output
        ...
        is ("b01000_00_101_01100_11".U) {
            // SRA instruction https://msyksphinz-self.github.io/riscv-isadoc/html/rvi.html#sra
            ...
            // Set up ALU
            alu.io.operation := "b1001".U // Logical right shift operation
            alu.io.signed := false.B
            alu.io.a := rs1_value
            alu.io.b := rs2_value
            val alu_result = alu.io.output
        ...
        is ("b00000_00_110_01100_11".U) {
            // OR instruction https://msyksphinz-self.github.io/riscv-isadoc/html/rvi.html#or
        ...
        is ("b00000_00_111_01100_11".U) {
            // AND instruction https://msyksphinz-self.github.io/riscv-isadoc/html/rvi.html#and
        ...
        is ("b000_00000_11".U) {
            // LB instruction https://msyksphinz-self.github.io/riscv-isadoc/html/rvi.html#lb
            ...
            // Execute LB: x[rd] = sext(M[x[rs1] + sext(offset)][7:0])
            ...
            // Sign extend offset
            val sext_offset = Cat(Fill(20, immediate(11)), immediate) // sext(offset)

            // Intermediate
            val _sum = rs1+sext_offset
            // Final Sign-extension
            val result = Cat(Fill(24, _sum(7)), _sum(7,0))

            regFile.io.write_addr := rd
            regFile.io.write_enable := true.B
            regFile.io.in := result
        }
    }
}
